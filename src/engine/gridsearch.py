...
def optuna_param_grid(
    ...
    if model_name == "rfc":  # same types of parameters
        ...
        for i in range(7):
            ...
            params[f"smote_{i + 1}"] = trial.suggest_int(
                f"smote_{i + 1}", resampling_repart[i][0], resampling_repart[i][1]
            )

            params[f"clust_{i + 1}"] = trial.suggest_int(
                f"clust_{i + 1}",
                min(max_clust, params[f"smote_{i + 1}"]),
                min(max_clust, params[f"smote_{i + 1}"])
            )
        ...
        error_msg = f"Unable to create optuna gridsearch for {model_name}."
